...
import { ArrowLeft, Home } from "lucide-react"; // Using Lucide React icons
...
function MessageDisplay() {
  ...
  return (
    <div className="absolute flex flex-col justify-center items-center w-[90%] h-[90%] z-[100]">
      ...
      >
        <div className="text-h2 font-semibold text-foreground m-[1%]">
          Page Not Found
        </div>
        <div className="text-h1 font-bold text-foreground m-[1%]">
          404
        </div>
        <div className="text-body w-1/2 min-w-[40%] text-center text-foreground m-[1%]">
          The page you are looking for might have been removed, had its name changed, or is temporarily unavailable.
        </div>
        <div className="flex gap-6 mt-8">
          <button
            onClick={() => router.back()} // Go back using Next.js router
            className="text-foreground border-2 border-foreground hover:bg-foreground hover:text-background transition-all duration-300 ease-in-out px-6 py-2 h-auto text-body font-medium flex items-center gap-2 hover:scale-105"
          >
            <ArrowLeft className="transition-transform group-hover:translate-x-1" size={20} />
            Go Back
          </button>
          <button
            onClick={() => router.push("/")} // Go home using Next.js router
            className="bg-foreground text-background hover:bg-muted-foreground transition-all duration-300 ease-in-out px-6 py-2 h-auto text-body font-medium flex items-center gap-2 hover:scale-105"
          >
            <Home className="transition-transform group-hover:translate-x-1" size={20} />
            Go Home
          </button>
        </div>
      </div>
